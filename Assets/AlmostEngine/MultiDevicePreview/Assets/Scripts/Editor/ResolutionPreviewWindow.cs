...
namespace AlmostEngine.Preview
{
	public class ResolutionPreviewWindow : ResolutionWindowBase
	{
		...
		[MenuItem ("Window/Multi Device Preview and Gallery/Device Preview")]
		static void Init ()
		{
...
//
			...
			int selected = EditorGUILayout.Popup (m_ConfigAsset.m_Selected, names.ToArray ());
			...
			float zoom = EditorGUILayout.Slider (m_ConfigAsset.m_PreviewZoom, 0.05f, 2f);
		...
		void DrawResolutionPreview (Rect pos, ScreenshotResolution resolution)
		{
			...
			if (resolution.m_Texture != null) {
				EditorGUI.DrawPreviewTexture (pos, resolution.m_Texture);
			} else {
		...
		{
			InitTempManager ();
			m_Manager.StartCoroutine (UpdateCoroutine (new List<ScreenshotResolution>{ m_SelectedResolution }));
		}
