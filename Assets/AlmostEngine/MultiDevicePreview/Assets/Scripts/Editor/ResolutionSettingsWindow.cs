...
namespace AlmostEngine.Preview
{
	public class ResolutionSettingsWindow : EditorWindow
	{
		...
		[MenuItem ("Window/Multi Device Preview and Gallery/Settings")]
		...
		{
			PreviewConfigAsset asset;
			PreviewConfigAsset[] objs = Resources.LoadAll<PreviewConfigAsset> ("");
			if (objs.Length == 0) {
				string path = "Assets/Resources/MultiDevicePreviewConfig.asset";
				Debug.Log ("Creating new config file at " + path);
				asset = ScriptableObject.CreateInstance<PreviewConfigAsset> ();
		...
		{
			Texture2D[] objs = Resources.LoadAll<Texture2D> ("");
			if (objs.Length != 0) {
				foreach (Texture2D t in objs) {
					if (t.name == "preview_logo") {
						return t;
					}
				}
			}
		...
		void OnGUI ()
		{
			...
			EditorGUILayout.Separator ();
			EditorGUILayout.HelpBox ("HOTKEYS. Note that you can use the Tools menu item hotkeys to Update the preview and Export the screenshots to files. You can customize them by editing the UpdateDevicePreviewMenuItem.cs and ExportDevicePreviewMenuItem.cs scripts.", MessageType.Info);
		...
		void DrawLogo ()
		{
			...
			Rect pos = new Rect (m_WindowWidth / 2 - logoSize / 2, 10, logoSize, logoSize);

			if (m_Logo != null) {
				EditorGUI.DrawPreviewTexture (pos, m_Logo);
			}
			EditorGUILayout.LabelField ("", GUILayout.MinHeight (logoSize + 10));
		}
...
//
//						EditorGUILayout.PropertyField (serializedObject.FindProperty ("m_MarginHorizontal"));
//						EditorGUILayout.PropertyField (serializedObject.FindProperty ("m_MarginVertical"));
//
//						EditorGUILayout.Separator ();
//
//						EditorGUILayout.PropertyField (serializedObject.FindProperty ("m_GalleryPaddingVertical"));
//						EditorGUILayout.PropertyField (serializedObject.FindProperty ("m_GalleryTextHeight"));
//						EditorGUILayout.PropertyField (serializedObject.FindProperty ("m_GalleryBorderSize"));
//
//
//
//						EditorGUI.indentLevel--;
...
//
			...
			EditorGUILayout.BeginVertical (GUI.skin.box);
			m_ConfigAsset.m_Config.m_ShowUtils = EditorGUILayout.Foldout (m_ConfigAsset.m_Config.m_ShowUtils, "Utils".ToUpper ());
